...
**Related guides:** [Claude Skills vs MCP Servers — when to use which](/blog/claude-skills-vs-mcp-servers) · [Slack MCP Server setup](/blog/slack-mcp-server-setup-guide) · [Supabase MCP Server setup](/blog/supabase-mcp-server-setup-guide) · [Playwright MCP Server setup](/blog/playwright-mcp-server-complete-guide) · [GitHub MCP Server setup](/blog/github-mcp-server-setup-guide)
...
`,
    ...
    faqs: [
      { question: "What is the GitHub MCP Server?", answer: "The GitHub MCP Server is GitHub's official open-source Model Context Protocol server. It lets AI assistants such as Claude, Cursor and GitHub Copilot read repositories, manage issues and pull requests, and inspect GitHub Actions runs through natural language." },
      { question: "Is the GitHub MCP Server free?", answer: "Yes. It is open source under the MIT license and the hosted remote server at api.githubcopilot.com/mcp/ is free with a GitHub account. Some remote-only tools, such as the Copilot coding agent, require a Copilot plan." },
      { question: "Should I use the remote or local GitHub MCP Server?", answer: "Use the remote server unless you use Claude Desktop, GitHub Enterprise Server, or an isolated network. The remote server needs no installation and updates automatically; it became generally available on September 4, 2025." },
      { question: "How do I add the GitHub MCP Server to Claude Code?", answer: "Run: claude mcp add --transport http github https://api.githubcopilot.com/mcp/ --header \"Authorization: Bearer YOUR_GITHUB_PAT\" and check the connection with /mcp inside Claude Code." },
      { question: "Is it safe to give an AI access to my GitHub repositories?", answer: "Use a fine-grained personal access token limited to specific repositories, grant only the permissions you need, set an expiry date, and start with read-only mode for the first week." },
      { question: "Why do older tutorials use npx @modelcontextprotocol/server-github?", answer: "That npm package was deprecated in April 2025 when GitHub took over the official server. Use the remote URL or the ghcr.io/github/github-mcp-server Docker image instead." },
    ],
  },
];
